...
//! that touch the native child window/view marshal onto the main/UI thread
//! via `on_main_thread` (Win32/GTK/AppKit are all single-thread-affine for
//! this). `Mpv` handle calls (play/pause/seek/volume) skip that - the
...
use crate::mpv_player::{self, window_impl, EmbeddedMpvSession, MpvCapability, MpvSession};
use crate::state::AppState;

/// Runs `f` on the main/UI thread and awaits its result. Needed for every
/// native window/view-mutating call - see the module doc comment.
...
pub async fn mpv_start_session(
    ...
    let handle = on_main_thread(&app, move || {
        ...
        let bounds = window_impl::WindowBounds {
        ...
        #[cfg(windows)]
        {
            ...
            window_impl::create(app_for_thread.clone(), sid.clone(), parent, bounds).map_err(CommandError::Api)
        }
        #[cfg(target_os = "linux")]
        {
            let gtk_window = main_window.gtk_window().map_err(|e| CommandError::Internal(format!("failed to get main GTK window: {e}")))?;
            let default_vbox = main_window.default_vbox().map_err(|e| CommandError::Internal(format!("failed to get main window's vbox: {e}")))?;
            window_impl::create(app_for_thread.clone(), sid.clone(), gtk_window, default_vbox, bounds).map_err(CommandError::Api)
        }
        #[cfg(target_os = "macos")]
        {
            let ns_view = main_window.ns_view().map_err(|e| CommandError::Internal(format!("failed to get main window's NSView: {e}")))?;
            window_impl::create(app_for_thread.clone(), sid.clone(), ns_view, bounds).map_err(CommandError::Api)
        }
    })
    .await?;

    let session = match MpvSession::start(app.clone(), session_id.clone(), url, title, handle.clone(), start_position_seconds) {
        Ok(session) => session,
        Err(e) => {
...
/// surface never shrinks for overlay UI (see `window_impl::set_bounds`).
/// `exclude_rects` are the parts overlay HTML currently owns (control bar,
/// close button) - see `window_impl::set_region`.
#[tauri::command]
pub async fn mpv_set_bounds(
    ...
    let handle = session.native_handle();
    let app_for_thread = app.clone();
    on_main_thread(&app, move || {
        let main_window = app_for_thread.get_webview_window("main").ok_or_else(|| CommandError::Internal("main window not found".into()))?;
        let scale = main_window.scale_factor().unwrap_or(1.0);
        let to_physical = |r: &RectDto| window_impl::WindowBounds {
        ...
        let exclude: Vec<window_impl::WindowBounds> = exclude_rects.iter().map(to_physical).collect();
        window_impl::set_bounds(handle.clone(), bounds);
        window_impl::set_region(handle, bounds, &exclude);
        Ok(())
    })
    .await
}
